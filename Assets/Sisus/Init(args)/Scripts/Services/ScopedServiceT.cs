...
namespace Sisus.Init
{
	...
	internal static class ScopedService<TService>
	{
		...
		#else
		[Preserve] public static readonly List<Instance> Instances = new List<Instance>();
		#endif
		...
		public static bool Add([NotNull] TService service, Clients clients, [NotNull] Component container)
		{
			#if UNITY_EDITOR
		...
		public static bool Remove([NotNull] TService service, Clients clients, [NotNull] Component container)
		{
			#if UNITY_EDITOR
			var instances = EditorOnly.ThreadSafe.Application.IsPlaying ? instancesInPlayMode : instancesInEditMode;
			foreach(var instance in instances.Keys)
			{
				if(ReferenceEquals(instance.service, service) && ReferenceEquals(instance.registerer, container))
				{
        ...
        [Preserve] private static void EnsureAOTPlatformSupport() => ServiceUtility.EnsureAOTPlatformSupportForService<TService>();
        #endif
		...
		internal readonly struct Instance : IEquatable<Instance>
		{
			...
			public readonly Transform Transform => registerer != null ? registerer.transform : null;
			public readonly Scene Scene => registerer != null ? registerer.gameObject.scene : default;

			public Instance([NotNull] TService service, Clients clients, [NotNull] Component registerer)
			{
