...
namespace Sisus.Init.Serialization
{
    ...
    public sealed class _Type : ISerializationCallbackReceiver, IValueProvider<Type>
    {
        ...
        public _Type([CanBeNull] string assemblyQualifiedTypeName, string guid = null)
        {
        ...
        /// <param name="guid"> (Optional:) The GUID of the script asset in which the wrapped type is defined. </param>
        public _Type([CanBeNull] Type value, string guid = null)
        {
            Value = value;
            scriptAssetGuid = guid;
        }
        ...
        public override string ToString() => (Value == null || Value.FullName == null) ? "None" : Value.Name;
        ...
        {
            if(typeNotFoundWarningHandledFor.Contains(TypeNameAndAssembly))
            {
                return;
            }

            Debug.LogWarning($"'{TypeNameAndAssembly}' was referenced but such type was not found.");
            typeNotFoundWarningHandledFor.Add(TypeNameAndAssembly);
        }
        ...
        public static bool operator ==(_Type a, _Type b) => a?.Equals(b) ?? ReferenceEquals(b, null);
        ...
        private static string GetTypeNameAndAssembly(Type type)
        {
            GuardAgainstUnserializable(type);
            return type != null ? $"{type.FullName}, {type.Assembly.GetName().Name}" : "";
        }

        private static void GuardAgainstUnserializable(Type type)
        {
            if(type != null && type.FullName == null)
            {
