...
{
	/// <summary>
	/// A base class for a component that can be used to specify the five arguments used to asynchronously initialize an object that implements
	...
	public abstract class AsyncInitializerBase<TClient, TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument> : MonoBehaviour, IInitializer<TClient, TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument>, IValueProvider<TClient>
		#if UNITY_EDITOR
		...
		where TClient : MonoBehaviour, IInitializable<TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument>
	{
		...
		bool IInitializerEditorOnly.HasNullArguments => IsNull(FirstArgument) || IsNull(SecondArgument) || IsNull(ThirdArgument) || IsNull(FourthArgument) || IsNull(FifthArgument);
		...
		public TClient InitTarget()
		{
			...
			if(TryInitTarget(FirstArgument, SecondArgument, ThirdArgument, FourthArgument, FifthArgument))
			{
				DestroySelf();
			}
		...
		[NotNull]
		protected virtual bool TryInitTarget(TFirstArgument firstArgument, TSecondArgument secondArgument, TThirdArgument thirdArgument, TFourthArgument fourthArgument, TFifthArgument fifthArgument)
        {
			if(firstArgument == Null || secondArgument == Null || thirdArgument == Null || fourthArgument == Null || fifthArgument == Null)
			{
				return false;
			}
		...
		{
			while(!TryInitTarget(FirstArgument, SecondArgument, ThirdArgument, FourthArgument, FifthArgument))
			{
				yield return null;
			}
