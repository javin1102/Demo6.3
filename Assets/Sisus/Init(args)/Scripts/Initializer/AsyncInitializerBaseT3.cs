...
{
	/// <summary>
	/// A base class for a component that can be used to specify the three arguments used to asynchronously initialize an object that implements
	...
	public abstract class AsyncInitializerBase<TClient, TFirstArgument, TSecondArgument, TThirdArgument> : MonoBehaviour, IInitializer<TClient, TFirstArgument, TSecondArgument, TThirdArgument>, IValueProvider<TClient>
		#if UNITY_EDITOR
		...
		where TClient : MonoBehaviour, IInitializable<TFirstArgument, TSecondArgument, TThirdArgument>
	{
		...
		bool IInitializerEditorOnly.HasNullArguments => IsNull(FirstArgument) || IsNull(SecondArgument) || IsNull(ThirdArgument);
		...
		public TClient InitTarget()
		{
			...
			if(TryInitTarget(FirstArgument, SecondArgument, ThirdArgument))
			{
				DestroySelf();
			}
		...
		[NotNull]
		protected virtual bool TryInitTarget(TFirstArgument firstArgument, TSecondArgument secondArgument, TThirdArgument thirdArgument)
        {
		...
		{
			while(!TryInitTarget(FirstArgument, SecondArgument, ThirdArgument))
			{
				yield return null;
			}
