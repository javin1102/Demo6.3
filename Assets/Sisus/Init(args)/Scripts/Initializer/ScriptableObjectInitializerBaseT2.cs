...
namespace Sisus.Init
{
	...
	/// initialize a scriptable object that implements <see cref="IInitializable{TFirstArgument, TSecondArgument}"/>.
	/// <para>
	...
	/// </para>
	/// <para>
	/// The client receives the arguments via the <see cref="IInitializable{TFirstArgument, TSecondArgument}.Init">Init</see>
	...
	public abstract class ScriptableObjectInitializerBase<TClient, TFirstArgument, TSecondArgument> : ScriptableObject,
		...
		where TClient : ScriptableObject, IInitializable<TFirstArgument, TSecondArgument>
	{
		...
		protected virtual TClient InitTarget(TFirstArgument firstArgument, TSecondArgument secondArgument)
        {
            if(target == null)
            {
                return target = Create.Instance<TClient, TFirstArgument, TSecondArgument>(firstArgument, secondArgument);
            }
			
			if(target is ScriptableObject<TFirstArgument, TSecondArgument> monoBehaviourT)
			{
