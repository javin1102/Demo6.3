...
namespace Sisus.Init.Internal
{
	internal static class InitializerUtility
	{
		...
		#if UNITY_EDITOR
		internal static void ResetArgument<TArgument, TInitializer>(ref TArgument argument, int argumentIndex, bool canAutoInitArguments, [NotNull] TInitializer initializer) where TInitializer : Object
		{
		...
		internal static void ResetInitializer<TClient>([NotNull] Object initializer, ref TClient client)
        {
			if(!(initializer is Component initializerComponent))
			{
				return;
			}

			if(typeof(Component).IsAssignableFrom(typeof(TClient)))
			{
	            client = (TClient)FindComponentWithoutInitializer(initializerComponent.gameObject, typeof(TClient));
			}

            if(!(client is Component clientComponent) || clientComponent.gameObject != initializerComponent.gameObject)
            {
                return;
            }
			...
			{
				MoveInitializerAboveItsClient(initializerComponent, clientComponent);
			}
        }

		private static object FindComponentWithoutInitializer(GameObject gameObject, Type componentType)
		{
			var options = gameObject.GetComponents(componentType);
			...
			return Find.In(gameObject, componentType, Including.Children | Including.Parents | Including.Scene | Including.Inactive);
		}
		...
		{
			if(IsNull(initializer, argument) || IsServiceDefiningType<TArgument>() || Service.TryGetFor<TArgument>(initializer.gameObject, out _))
			{
				return true;
			}
		...
		internal static void Validate(IInitializerEditorOnly initializer)
		{
			...
			component.gameObject.GetComponents(initializers);
		...
		internal static bool IsNull<TArgument>(Object initializer, Any<TArgument> argument) => !argument.GetHasValue(initializer, Context.MainThread);

		internal static void UpdateHideFlags(IInitializer initializer)
		{
			if(!(initializer is Component initializerComponent))
			{
				return;
			...
			var setHideFlags = !(initializer.Target is Component clientComponent) || clientComponent.gameObject != initializerComponent.gameObject ? HideFlags.None : HideFlags.HideInInspector;
			if(setHideFlags != initializerComponent.hideFlags)
			{
				Undo.RecordObject(initializerComponent, "Update HideFlags");
				initializerComponent.hideFlags = setHideFlags;
			}
		}
		...
		internal static void ValidateOnMainThread<TInitializer>(TInitializer initializer) where TInitializer : Object, IInitializerEditorOnly
		{
			if(initializer == null)
			{
				return;
			...
			GameObject gameObject = initializer is Component component ? component.gameObject : null;
			Object selectionRoot = gameObject != null ? gameObject : initializer as ScriptableObject as Object;

			if(Array.IndexOf(Selection.gameObjects, selectionRoot) == -1 && (gameObject == null) || gameObject.activeInHierarchy)
			{
				...
				Debug.LogWarning(message, initializer.Target ?? initializer);
				initializer.NullGuardFailedMessage = message;
				return;
			}
			...
			static string GetClientComponentTypeName(IInitializer initializer)
			{
		...
		internal static TArgument HandleReset<TInitializer, TClient, TArgument>(TInitializer initializer, ref TClient target, TArgument argument,
			OnResetHandler<TArgument> overridableOnReset) where TInitializer : Object, IInitializerEditorOnly where TClient : Object
		{
		...
		internal static (TFirstArgument firstArgument, TSecondArgument secondArgument) HandleReset<TInitializer, TClient, TFirstArgument, TSecondArgument>
			...
			OnResetHandler<TFirstArgument, TSecondArgument> overridableOnReset) where TInitializer : Object, IInitializerEditorOnly where TClient : Object
		{
			ResetInitializer(initializer, ref target);
		...
		internal static (TFirstArgument firstArgument, TSecondArgument secondArgument, TThirdArgument thirdArgument)
			...
			OnResetHandler<TFirstArgument, TSecondArgument, TThirdArgument> overridableOnReset) where TInitializer : Object, IInitializerEditorOnly where TClient : Object
		{
			ResetInitializer(initializer, ref target);
		...
		internal static (TFirstArgument firstArgument, TSecondArgument secondArgument, TThirdArgument thirdArgument, TFourthArgument fourthArgument)
			...
			OnResetHandler<TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument> overridableOnReset) where TInitializer : Object, IInitializerEditorOnly where TClient : Object
		{
			ResetInitializer(initializer, ref target);
		...
		internal static (TFirstArgument firstArgument, TSecondArgument secondArgument, TThirdArgument thirdArgument, TFourthArgument fourthArgument, TFifthArgument fifthArgument)
			...
			OnResetHandler<TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument> overridableOnReset) where TInitializer : Object, IInitializerEditorOnly where TClient : Object
		{
			ResetInitializer(initializer, ref target);
		...
		internal static (TFirstArgument firstArgument, TSecondArgument secondArgument, TThirdArgument thirdArgument, TFourthArgument fourthArgument, TFifthArgument fifthArgument, TSixthArgument sixthArgument)
			...
			OnResetHandler<TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument, TSixthArgument> overridableOnReset) where TInitializer : Object, IInitializerEditorOnly where TClient : Object
		{
			ResetInitializer(initializer, ref target);
		...
		private static IEnumerable<Type> GetInitializerTypes(Component component)
		{
