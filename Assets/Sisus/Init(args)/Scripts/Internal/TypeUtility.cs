...
namespace Sisus.Init.Internal
{
    public static class TypeUtility
    {
		...
		[NotNull]
		internal static IEnumerable<(Type classWithAttribute, TAttribute[] attributes)> GetTypesWithAttribute<TAttribute>([NotNull] Assembly mustReferenceAssembly, [CanBeNull] Assembly ignoreAssembly) where TAttribute : Attribute
		{
			#if UNITY_EDITOR
			foreach(var type in UnityEditor.TypeCache.GetTypesWithAttribute<TAttribute>())
			{
		...
		public static bool IsBaseType([NotNull] Type type) => type.IsGenericType switch
        {
		...
		public static bool DerivesFromGenericBaseType([NotNull] Type type)
		{
			while((type = type.BaseType) != null)
			{
				if(type.IsGenericType && genericBaseTypes.Contains(type.GetGenericTypeDefinition()))
				{
					return true;
				}
			}
