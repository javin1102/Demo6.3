...
namespace Sisus.Init.Reflection
{
	...
	public static class InjectionUtility
	{
		...
		[CanBeNull]
		internal static bool TryGetInitArgumentTargetMember([NotNull] Type clientType, [NotNull] Type argumentType, int parameterIndex, out MemberInfo result)
        {
			...
			for(var type = clientType; !IsNullOrBaseType(type); type = type.BaseType)
			{
				int index = 0;
				foreach(var member in clientType.GetMembers(AnyDeclaredInstance))
				{
