...
namespace Sisus.Init.EditorOnly.Internal
{
	...
	/// </summary>
	[CustomPropertyDrawer(typeof(Any<>), useForChildren: true)]
	#if ODIN_INSPECTOR
	// Because Odin inspector does not support custom property drawers
	// targeting open generic types, we need to target a non-generic
	// interface which Any{T} implements instead as a workaround.
    ...
    internal sealed class AnyPropertyDrawer : PropertyDrawer, IDisposable
    {
		private sealed class State
		{
			...
			/// or if the user has selected "Object Reference" from the type dropdown of an interface or object field.
			/// </para>
			/// </summary>
		...
		internal static bool TryGetAssignableTypeFromDraggedObject(Object draggedObject, Type anyType, Type valueType, out Type assignableType)
        {
			...
			if(draggedObject is GameObject gameObject && valueType != typeof(GameObject) && valueType != typeof(Object) && valueType != typeof(object))
            {
                ...
                {
					if(TryGetAssignableTypeFromDraggedObject(component, anyType, valueType, out assignableType))
                    {
						return true;
                    }
                }
		...
		public static bool IsService(Object targetObject, Type valueType) => ServiceUtility.IsServiceDefiningType(valueType) || ServiceUtility.ServiceExists(targetObject, valueType);
        ...
        {
			state.referenceProperty = anyProperty.FindPropertyRelative(nameof(Any<object>.reference));
			state.valueProperty = anyProperty.FindPropertyRelative(nameof(Any<object>.value));
			...
			argumentIsService = IsService(anyProperty.serializedObject.targetObject, valueType);
			...
			valueHasChildProperties = !valuePropertyIsNull && state.valueProperty.Copy().Next(true);
		...
		private IEnumerable<Type> GetTypeOptions([CanBeNull] SerializedProperty valueProperty, Type valueType)
		{
		...
		private bool TryGetAssignableTypeFromDraggedObject(Object draggedObject, out Type assignableType) => TryGetAssignableTypeFromDraggedObject(draggedObject, anyType, valueType, out assignableType);
			...
			bool isService, bool canBeNonUnityObject, bool? draggedObjectIsAssignable)
        {
			...
			if(drawAsObjectField)
            {
				...
				#if UNITY_LOCALIZATION
				if(referenceValue is LocalizedString localizedString)
				{
					if(referenceSerializedObject == null)
					{
						referenceSerializedObject = new SerializedObject(localizedString);
					}
					else
					{
						referenceSerializedObject.Update();
					}

					var localizedStringProperty = referenceSerializedObject.FindProperty("value");

					EditorGUI.BeginChangeCheck();

					position.height = EditorGUI.GetPropertyHeight(localizedStringProperty, label, true);
					EditorGUI.indentLevel = 0;
					EditorGUI.PropertyField(position, localizedStringProperty, label, true);
					EditorGUI.indentLevel = indentLevelWas;

					referenceSerializedObject.ApplyModifiedProperties();
        ...
        //public float GetPropertyHeight(SerializedProperty property, GUIContent label) => Height;
		...
		private static void SetUserSelectedValueType(State state, Type type)
        {
			...
			if(valueProperty.propertyType != SerializedPropertyType.ManagedReference)
			{
				...
				else if(type == typeof(LocalizedString))
				{
					...
					state.referenceProperty.objectReferenceValue = localizedString;
				}
				#endif
		...
		private void HandleDispose()
		{
			...
			if(referenceSerializedObject != null)
			{
				referenceSerializedObject.Dispose();
