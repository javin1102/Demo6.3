...
namespace Sisus.Init.EditorOnly.Internal
{
	internal static class ServiceTagUtility
	{
		...
		internal static void Ping(Type serviceDefiningType, Component component)
		{
			try
			{
				var instance = ServiceUtility.Get(component, serviceDefiningType) as Object;
				if(instance != null)
				{
		...
		internal static void SelectAllReferencesInScene(object service)
		{
		...
		internal static IEnumerable<Type> GetServiceDefiningTypes(object service)
		{
			currentDefiningTypes.Clear();

			GetServiceDefiningTypes(service, service.GetType(), currentDefiningTypes);

			// We want to display the service tag on wrappers and initializers as well when their target is a service.
			if(service is IValueProvider valueProvider && valueProvider.Value is object providedValue)
			{
				GetServiceDefiningTypes(service, providedValue.GetType(), currentDefiningTypes);
			}

			return currentDefiningTypes;
		}

		private static void GetServiceDefiningTypes(object service, Type type, HashSet<Type> currentDefiningTypes)
		{
			...
			{
				if(ServiceUtility.TryGetFor(service, type, out object someService) && ReferenceEquals(someService, service))
				{
			...
			foreach(var t in type.GetInterfaces())
			{
				if(ServiceUtility.TryGetFor(service, t, out object someService) && ReferenceEquals(someService, service))
				{
					currentDefiningTypes.Add(t);
				}
			}
