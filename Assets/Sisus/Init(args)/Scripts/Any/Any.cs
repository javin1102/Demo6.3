		...
		#endif
	{
		...
		public T ValueOrDefault
		{
			get
			{
				...
				if(Service.TryGet(out T service))
                {
					return service;
                }
		...
		public T GetValueOrDefault<TClient>([NotNull] TClient client, Context context = Context.MainThread)
        {
			...
			if(Service.TryGetFor(client, out T service))
			{
				return service;
			}
		...
		public bool GetHasValue<TClient>(TClient client, Context context = Context.MainThread)
        {
			...
			if(reference is LocalizedString localizedString)
			{
				return localizedString.HasValue;
			}
			#endif

			#if UNITY_EDITOR
			if(reference is CrossSceneReference crossSceneReference)
			{
				return crossSceneReference.HasValue;
			}
        ...
        public override bool Equals(object obj)
		{
            ...
            return obj.Equals(ValueOrDefault);
		}
