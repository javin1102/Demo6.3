...
Shader "AmplifyShaderPack/Triplanar Projection"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			inline float4 TriplanarSampling343( sampler2D topTexMap, const float4 topST, sampler2D midTexMap, const float4 midST, sampler2D botTexMap, const float4 botST, float3 worldPos, float3 worldNormal, float falloff, float2 tiling, float3 normalScale, float3 index )
			{
			...
			inline float3 TriplanarSampling345( sampler2D topTexMap, const float4 topST, sampler2D midTexMap, const float4 midST, sampler2D botTexMap, const float4 botST, float3 worldPos, float3 worldNormal, float falloff, float2 tiling, float3 normalScale, float3 index )
			{
				float3 projNormal = ( pow( abs( worldNormal ), falloff ) );
				projNormal /= ( projNormal.x + projNormal.y + projNormal.z ) + 0.00001;
				float3 nsign = sign( worldNormal );
		...
		Pass
		{
			...
			float4 Frag(PackedVaryingsMeshToPS packedInput  ) : SV_Target
			{
				...
				float3 triplanar345 = TriplanarSampling345( _TopNormal, _TopNormal_ST, _TriplanarNormal, _TriplanarNormal_ST, _TriplanarNormal, _TriplanarNormal_ST, ase_positionWS, ase_normalWS, _CoverageFalloff, float2( 1,1 ), float3( 1,1,1 ), float3(0,0,0) );
				float3 tanTriplanarNormal345 = mul( ase_worldToTangent, triplanar345 );
		...
		Pass
		{
					 ...
					 )
			{
				...
				float3 ase_bitangentWS = packedInput.ase_texcoord5.xyz;
				float3x3 ase_worldToTangent = float3x3( ase_tangentWS, ase_bitangentWS, ase_normalWS );
				float3 triplanar345 = TriplanarSampling345( _TopNormal, _TopNormal_ST, _TriplanarNormal, _TriplanarNormal_ST, _TriplanarNormal, _TriplanarNormal_ST, PositionWS, ase_normalWS, _CoverageFalloff, float2( 1,1 ), float3( 1,1,1 ), float3(0,0,0) );
				float3 tanTriplanarNormal345 = mul( ase_worldToTangent, triplanar345 );
...
ASEEND*/
//CHKSM=68B4C338AB2A52D24199F4999D1F7F816AB01B3F